...
export function showUserDetailModal(userDetail) {
            ...
            <div class="modal-content user-detail-modal">
                ...
                <div class="modal-body">
                    ...
                    <div class="user-info-section">
                        ...
                        <!-- 个人简介展示开关 -->
                        <div class="show-bio-toggle">
                            <div class="toggle-content">
                                <div class="toggle-label">
                                    <div class="toggle-title-row">
                                        <span class="toggle-title">向AI展示个人简介</span>
                                        <div class="info-icon" id="showBioInfoBtn">?</div>
                                    </div>
                                    <span class="toggle-description">开启后AI会了解你的个人设定</span>
                                </div>
                            </div>
                            <div class="toggle-switch ${userDetail.showBio ? 'active' : ''}" id="showBioToggle">
    ...
    // 绑定 showBio 开关和功能说明弹窗事件
    bindShowBioEvents(modal, userDetail);
...
// 绑定 showBio 开关和功能说明弹窗事件
function bindShowBioEvents(modal, userDetail) {
    // 绑定开关切换事件
    const showBioToggle = modal.querySelector('#showBioToggle');
    if (showBioToggle) {
        showBioToggle.addEventListener('click', (e) => {
            e.currentTarget.blur();
            handleToggleShowBio(modal, userDetail);
        });
    }

    // 绑定问号按钮事件
    const showBioInfoBtn = modal.querySelector('#showBioInfoBtn');
    if (showBioInfoBtn) {
        showBioInfoBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            e.currentTarget.blur();
            showBioInfoModal(modal);
        });
    }
}

// 切换 showBio 开关
async function handleToggleShowBio(modal, userDetail) {
    const showBioToggle = modal.querySelector('#showBioToggle');
    const currentShowBio = userDetail.showBio || false;
    const newShowBio = !currentShowBio;

    try {
        showBioToggle.style.pointerEvents = 'none';

        // 调用后端 API
        const response = await apiRequest(`${API_USERSERVICE_URL}/api/users/show-bio?showBio=${newShowBio}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (response.ok) {
            // 更新本地状态
            userDetail.showBio = newShowBio;

            // 更新 UI
            if (newShowBio) {
                showBioToggle.classList.add('active');
            } else {
                showBioToggle.classList.remove('active');
            }

            showToast(newShowBio ? '已开启个人简介展示' : '已关闭个人简介展示');
        } else {
            const result = await response.json();
            showToast(`更新失败: ${result.message || result.error || '未知错误'}`, 'error');
        }
    } catch (error) {
        console.error('更新 showBio 失败:', error);
        showToast('网络错误，请检查网络连接', 'error');
    } finally {
        showBioToggle.style.pointerEvents = 'auto';
    }
}

// 显示功能说明弹窗
function showBioInfoModal(parentModal) {
    // 隐藏父模态框的焦点
    if (parentModal) {
        manageModalFocus(parentModal, 'hide');
    }
    ...
    // 创建独立的弹窗
    const bioInfoModal = document.createElement('div');
    bioInfoModal.className = 'info-modal';
    bioInfoModal.innerHTML = `
        <div class="info-modal-overlay">
            <div class="modal-content-inner" id="bioInfoModalContent">
                <div class="modal-header-inner">
                    <h3 class="modal-title-inner">个人简介设置说明</h3>
                    <button class="close-btn-inner" id="closeBioInfoBtn">×</button>
                </div>
                <div class="modal-body-inner">
                    <p class="modal-text">通过个人简介自由设定你的喜好或人设，开启后AI会根据你的设定调整回答风格。</p>
                    <p class="modal-examples">例如：</p>
                    <p class="modal-example">"我不喜欢啰嗦，请高度概括性的回答问题"</p>
                    <p class="modal-example">"我是一个绝世高手，任何人对我都充满了敬重与畏惧"</p>
                </div>
            </div>
        </div>
    `;

    document.body.appendChild(bioInfoModal);

    // 使用弹窗管理器注册弹窗
    modalManager.pushModal('bioInfoModal', bioInfoModal, 'userDetailModal', () => {
        // 恢复父模态框的焦点状态
        if (parentModal) {
            manageModalFocus(parentModal, 'restore');
        }
        document.body.removeChild(bioInfoModal);
    });

    // 绑定关闭事件
    const closeBtn = bioInfoModal.querySelector('#closeBioInfoBtn');
    const overlay = bioInfoModal.querySelector('.info-modal-overlay');

    const closeBioInfoModal = () => {
        modalManager.closeModal('bioInfoModal');
    };
